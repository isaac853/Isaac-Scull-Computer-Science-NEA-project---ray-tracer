...
{
    // 3D Transform class: rotation (quaternion) + translation, translation followed by a rotation
    class Transform
    {
    public:
        ...
        Transform(floating qx_, floating qy_, floating qz_, floating qw_,
            ...
            : qx(qx_), qy(qy_), qz(qz_), qw(qw_), tx(tx_), ty(ty_), tz(tz_) // inputted values
        {
        }
        ...
        Transform &inverse(Transform &result)
        {
            ...
            // to invert, you need to start with a backwards translation, relative to the rotation you have already done
            //non-trivial as forward rotation messed up coords

            // this is a shortcut to converting to a 4x3 matrix and then multiplying it by the translation vector
            // this saves a few multiplies
